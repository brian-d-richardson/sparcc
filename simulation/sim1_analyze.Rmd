...
We also vary which nuisance distributions are correct. In all settings, the MLE and semiparametric estimator assume $X \sim \textrm{exponential}(\alpha_1)$ and $C \sim \textrm{exponential}(\alpha_2)$ (which are special cases of the gamma distribution with shape parameter equal to 1). To simulate the scenarios where $X$ and/or $C$ (or neither) are misspecified, we do the following:
...
We estimate the nuisance distributions $\eta_1(x) \equiv f_{X}(x)$ and $\eta_2(c) \equiv f_{C}(c)$ with $\eta_1^*(x, \widehat\alpha_1)=\alpha_1^{-1}\exp(-x/\alpha_1)$, $\eta_2^{\dagger}(c, \widehat\alpha_2)=\alpha_2^{-1}\exp(-c/\alpha_2)$, where $\widehat\alpha_1 = \frac{\sum_{i=1}^n W_i}{\sum_{i=1}^n \Delta_i}$, $\widehat\alpha_2 = \frac{\sum_{i=1}^n W_i}{\sum_{i=1}^n 1-\Delta_i}$ are the MLEs for $\alpha_1, \alpha_2$.
...
sim.out.list <- lapply(
  ...
  FUN = function(clust) {
    cbind(clust,
          read.csv(paste0("sim_data/sim1/estetas_meanX1_shape_1_25/sd",
                          clust, ".csv")))
  })
...
make.est.plot(param. = 2, n. = 10000) + ylim(1.2, 2.5)
