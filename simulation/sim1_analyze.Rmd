...
* $X|Z \sim \textrm{gamma}(\theta_1, \theta_2)$,
* $C|Z \sim \textrm{gamma}(\gamma_1, \gamma_2)$,
...
We also vary which nuisance distributions are correct by modeling $X|Z$ (or $C|Z$) as gamma (correct) or exponential (incorrect).
...
sim.out.long %>% 
  filter(param == 1) %>% 
  group_by(q, n, specify.x.gamma, specify.c.gamma, method, trueB2, s2) %>% 
...
n.rep <- nrow(sim.out) /
  n_distinct(dplyr::select(sim.out, q, n, specify.x.gamma, specify.c.gamma, mx, mc, my, s2, trueB2))
...
make.est.plot <- function(param., q., est_cutoff = Inf) {
  ...
  sim.out.long %>% 
    ...
    geom_hline(aes(yintercept = B.true),
               ...
               linewidth = 0.6,
               color = pal_light[4]) +
    facet_nested(trueB2 ~ specify.x.gamma + specify.c.gamma,
                 scales = "free",
...
make.est.plot(param. = 4, q. = q[2])

```

```{r message = F}

make.est.plot(param. = 1, q. = q[3], est_cutoff = 0.2)
make.est.plot(param. = 2, q. = q[3], est_cutoff = 2)
make.est.plot(param. = 3, q. = q[3], est_cutoff = 1)
make.est.plot(param. = 4, q. = q[3], est_cutoff = 1)
...
tbl <- sim.out.long %>%
  group_by(n, q, s2, trueB2, specify.x.gamma, specify.c.gamma, method, param) %>% 
