...
# Purpose: develop variance estimator for MLE when X ~ beta
...
# check that derivative of X log-likelihood is close to zero
dlf <- d.log.fx(dat = dat, theta = x.params.hat, return.sums = F)
colSums(dlf)
...
# compare to std error from fitdistrplus function (expect very similar)
x.params.se
est$sd
